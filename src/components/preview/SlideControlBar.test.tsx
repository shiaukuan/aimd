...
import { SlideNavigationControls, SlideZoomControls } from '@/types/slidePreview';
...
describe('SlideControlBar', () => {
  ...
  describe('縮放控制', () => {
    ...
    it('應能點擊重置縮放按鈕', async () => {
      ...
      const resetZoomButton = screen.getByTestId('rotate-ccw').closest('button');
  ...
  describe('自訂樣式', () => {
    ...
    it('應包含預設佈局樣式', () => {
      ...
      const controlBar = document.querySelector('.flex.items-center.justify-between');
      expect(controlBar).toBeInTheDocument();
    });
  });
