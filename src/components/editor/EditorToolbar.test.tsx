...
describe('EditorToolbar', () => {
  it('should render all toolbar groups by default', () => {
    ...
    // 檢查標題按鈕
    expect(screen.getByTestId('toolbar-heading1')).toBeInTheDocument();
    expect(screen.getByTestId('toolbar-heading2')).toBeInTheDocument();
    expect(screen.getByTestId('toolbar-heading3')).toBeInTheDocument();
  ...
  it('should hide groups when corresponding props are false', () => {
    ...
    // 設定按鈕應該被隱藏
    expect(screen.queryByTestId('toolbar-settings')).not.toBeInTheDocument();
  });
