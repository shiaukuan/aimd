...
test.describe('Split Panel功能', () => {
  ...
  test('應該包含編輯器和預覽區域', async ({ page }) => {
    ...
    console.log(
      'Contains "Markdown 編輯器":',
      pageText?.includes('Markdown 編輯器')
    );
    ...
    // 然後檢查標題 - 如果不存在就跳過
    const editorTitleExists = await page
      .locator('h2')
      .filter({ hasText: 'Markdown 編輯器' })
      .count();
    const previewTitleExists = await page
      .locator('h2')
      .filter({ hasText: '預覽' })
      .count();
  ...
  test('應該可以拖拉調整面板大小', async ({ page }) => {
    ...
    // 獲取初始寬度
    const initialLeftWidth = await leftPanel.evaluate(
    ...
    // 檢查左面板寬度有改變
    const newLeftWidth = await leftPanel.evaluate(
      el => el.getBoundingClientRect().width
    );
  ...
  test('應該保持最小寬度限制', async ({ page }) => {
    ...
    // 檢查左面板沒有小於最小寬度（300px）
    const leftWidth = await leftPanel.evaluate(
      el => el.getBoundingClientRect().width
    );
  ...
  test('面板大小設定應該持久化', async ({ page }) => {
    ...
    // 記錄調整後的寬度
    const adjustedWidth = await leftPanel.evaluate(
      el => el.getBoundingClientRect().width
    );
    ...
    // 檢查寬度是否保持
    const persistedWidth = await leftPanel.evaluate(
      el => el.getBoundingClientRect().width
    );
