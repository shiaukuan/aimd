...
- `src/lib/__tests__/utils.test.ts` - 範例單元測試
...
```
**背景**: 基礎 Markdown 編輯器已實作完成。現在加入進階功能提升編輯體驗。

**任務**: 實作編輯器的進階功能，包括快捷鍵、自動儲存等

**技術要求**:
- 實作 Tab/Shift+Tab 縮排功能
- 加入 undo/redo 支援 (Ctrl+Z/Ctrl+Y)
- 實作自動儲存到 localStorage
- 加入編輯器設定選項

**實作步驟**:
1. 建立 `useEditorShortcuts` hook
2. 實作縮排/反縮排邏輯
3. 建立自動儲存機制
4. 加入編輯器設定管理
5. 更新 `MarkdownEditor` 整合新功能

**需要建立的檔案**:
- `src/hooks/useEditorShortcuts.ts` - 鍵盤快捷鍵
- `src/hooks/useAutoSave.ts` - 自動儲存
- `src/lib/editor-settings.ts` - 編輯器設定

**功能要求**:
- Tab 鍵增加縮排，Shift+Tab 減少縮排
- Ctrl+Z/Ctrl+Y 支援復原/重做
- 每 30 秒自動儲存到 localStorage
- 可設定 Tab 大小、自動換行等選項

**測試標準**:
- Tab 和 Shift+Tab 正確處理縮排
- Ctrl+Z/Y 可復原和重做編輯
- 重新整理頁面後內容保留
- 設定變更即時生效
```
...
```
**背景**: 編輯器功能已完善。現在實作編輯器與預覽區域的即時同步機制。

**任務**: 建立編輯器內容變更時的即時同步和錯誤處理
...
這個計劃將 Markdown 投影片產生器分解為 27 個可測試的小步驟，每個步驟都：
